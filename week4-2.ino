...
void loop() {
  ...
  //Player 1
  if(digitalRead(Button1_Pin) == HIGH && btn1_Pressed == false && gameend==false){
    btn1_Pressed = true;
    pos += 5; //角度加五度
  }
  ...
  //Player 2
  if(digitalRead(Button2_Pin) == HIGH && btn2_Pressed == false && gameend==false){
    btn2_Pressed = true;
    pos -= 5; //角度減五度
  }
  ...
  if(pos>=180||pos<=0){digitalWrite(ledpin,HIGH);gameend=true;}
  if(gameend==true&&digitalRead(Button1_Pin) == HIGH&&digitalRead(Button2_Pin) == HIGH){
  pos = 90;myservo.write(90);gameend=false;digitalWrite(ledpin,LOW);btn1_Pressed = false;btn2_Pressed = false;
  }
}
